...
export function greetingPrompt(persona?: PersonaInfo, sceneSummary?: string): string {
  ...
  return `${identity}
A new patient joined your live voice session. Give a brief warm opening (1-3 sentences, under 40 words). Welcome them, set the tone, invite sharing. Plain language only — no markdown, no labels. Don't state your name. Be spontaneous.${visionLine}`;
}
...
- Don't narrate non-verbal observations as fact ("I can see you're anxious"). Use hedged language ("it seems like something shifted", "I notice something might be coming up").
- Never claim to know their inner state.
- If self-harm is mentioned, urge professional help immediately.${arousalPacing}
Rules: Plain language only (no markdown/JSON). 2-4 short sentences. Stay in character.`;
}
